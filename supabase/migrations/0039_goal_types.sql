-- Extend client goals: performance, habit, milestone types + target dates
...
create index if not exists client_goals_exercise_id_idx
  on public.client_goals (exercise_id);
create index if not exists client_goals_program_id_idx
  on public.client_goals (program_id);

-- ---------------------------------------------------------------------------
-- Replace category field check
-- ---------------------------------------------------------------------------
...
alter table public.client_goals add constraint client_goals_category_fields check (
  (
    ...
    and performance_metric is null
    and habit_source is null
    and habit_frequency is null
    and habit_period is null
    and milestone_type is null
    and milestone_target_count is null
  )
  or (
    category::text = 'daily'
  ...
  or (
    ...
    and milestone_target_count is null
    and (
      performance_metric = 'powerlifting_total'
      or exercise_id is not null
    )
  )
  or (
    category::text = 'habit'
    and habit_source is not null
    and habit_frequency is not null
    and habit_frequency > 0
    ...
    and milestone_target_count is null
    and (
      habit_source <> 'nutrition_adherence'
  ...
  or (
    ...
    and milestone_target_count is not null
    and milestone_target_count > 0
    and metric is null
    and direction is null
    and target_amount is null
    and exercise_id is null
    ...
    and habit_period is null
    and (
      milestone_type <> 'program_completion'
...
comment on table public.client_goals is
  'Coach-set goals: composition, daily targets, performance, habits, and milestones.';
