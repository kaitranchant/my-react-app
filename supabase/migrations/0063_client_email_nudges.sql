-- Client email nudges: portal prefs + deduplication log (0063)
...
comment on column public.profiles.portal_notify_workout_reminders is
  'Email client on days they have a scheduled workout that has not started.';
comment on column public.profiles.portal_notify_check_in_reminders is
  'Email client when a check-in is due for their coach''s cadence.';
comment on column public.profiles.portal_notify_unread_digest is
  'Email client a daily digest when they have unread coach messages.';
