-- Fix gym delete when shared clients/teams belong to other coaches, and
-- support post-facility client-limit resolution for coaches falling back to Starter.

-- ---------------------------------------------------------------------------
-- profiles.needs_client_limit_resolution
-- ---------------------------------------------------------------------------

alter table public.profiles
  add column if not exists needs_client_limit_resolution boolean not null default false;

comment on column public.profiles.needs_client_limit_resolution is
  'When true, coach must archive down to their plan client limit or upgrade before continuing.';

-- ---------------------------------------------------------------------------
-- Allow gym owners to clear gym_id on any shared client (dissolve / unshare-all)
-- ---------------------------------------------------------------------------

create or replace function public.clients_gym_share_guard()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if TG_OP = 'UPDATE'
    and old.is_coach_self
    and new.is_coach_self
    and new.coach_id is not distinct from old.coach_id
    and new.gym_id is distinct from old.gym_id
    and new.user_id is not distinct from old.user_id
    and new.invite_status is not distinct from old.invite_status
    and new.invite_token is not distinct from old.invite_token
    and new.invite_expires_at is not distinct from old.invite_expires_at
    and (
      auth.uid() is null
      or (
        auth.uid() = old.coach_id
        and (new.gym_id is null or public.is_gym_member(new.gym_id))
      )
      or (
        new.gym_id is not null
        and exists (
          select 1
          from public.gym_members gm
          where gm.coach_id = old.coach_id
            and gm.gym_id = new.gym_id
            and gm.status = 'active'
        )
      )
    )
  then
    return new;
  end if;

  if auth.uid() is distinct from old.coach_id then
    if old.invite_status = 'pending'
      and new.invite_status = 'accepted'
      and old.user_id is null
      and new.user_id is not null
      and (auth.uid() is null or new.user_id = auth.uid())
      and new.coach_id is not distinct from old.coach_id
      and new.gym_id is not distinct from old.gym_id
      and new.is_coach_self is not distinct from old.is_coach_self
    then
      return new;
    end if;

    -- Gym owner reassigning primary coach for a shared client
    if new.coach_id is distinct from old.coach_id
      and old.gym_id is not null
      and not old.is_coach_self
      and new.gym_id is not distinct from old.gym_id
      and new.user_id is not distinct from old.user_id
      and new.invite_token is not distinct from old.invite_token
      and new.invite_status is not distinct from old.invite_status
      and new.invite_expires_at is not distinct from old.invite_expires_at
      and new.is_coach_self is not distinct from old.is_coach_self
      and exists (
        select 1
        from public.gym_members gm
        where gm.gym_id = old.gym_id
          and gm.coach_id = auth.uid()
          and gm.role = 'owner'
          and gm.status = 'active'
      )
      and exists (
        select 1
        from public.gym_members gm
        where gm.gym_id = old.gym_id
          and gm.coach_id = new.coach_id
          and gm.status = 'active'
      )
    then
      return new;
    end if;

    -- Gym owner clearing share (delete gym / dissolve): keep primary coach
    if new.gym_id is null
      and old.gym_id is not null
      and new.coach_id is not distinct from old.coach_id
      and new.user_id is not distinct from old.user_id
      and new.invite_token is not distinct from old.invite_token
      and new.invite_status is not distinct from old.invite_status
      and new.invite_expires_at is not distinct from old.invite_expires_at
      and new.is_coach_self is not distinct from old.is_coach_self
      and exists (
        select 1
        from public.gym_members gm
        where gm.gym_id = old.gym_id
          and gm.coach_id = auth.uid()
          and gm.role = 'owner'
          and gm.status = 'active'
      )
    then
      return new;
    end if;

    if new.coach_id is distinct from old.coach_id
      or new.gym_id is distinct from old.gym_id
      or new.user_id is distinct from old.user_id
      or new.invite_token is distinct from old.invite_token
      or new.invite_status is distinct from old.invite_status
      or new.invite_expires_at is distinct from old.invite_expires_at
      or new.is_coach_self is distinct from old.is_coach_self
    then
      raise exception 'Only the primary coach can change client ownership or sharing settings';
    end if;
  end if;

  if new.gym_id is distinct from old.gym_id and auth.uid() = old.coach_id then
    if new.gym_id is not null and not public.is_gym_member(new.gym_id) then
      raise exception 'You must be an active member of the gym to share this client';
    end if;
  end if;

  return new;
end;
$$;

-- ---------------------------------------------------------------------------
-- Allow gym owners to clear gym_id on shared teams
-- ---------------------------------------------------------------------------

create or replace function public.teams_gym_share_guard()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if auth.uid() is distinct from old.coach_id then
    if new.gym_id is null
      and old.gym_id is not null
      and new.coach_id is not distinct from old.coach_id
      and exists (
        select 1
        from public.gym_members gm
        where gm.gym_id = old.gym_id
          and gm.coach_id = auth.uid()
          and gm.role = 'owner'
          and gm.status = 'active'
      )
    then
      return new;
    end if;

    if new.coach_id is distinct from old.coach_id
      or new.gym_id is distinct from old.gym_id
    then
      raise exception 'Only the primary coach can change team ownership or sharing settings';
    end if;
  end if;

  if new.gym_id is distinct from old.gym_id and auth.uid() = old.coach_id then
    if new.gym_id is not null and not public.is_gym_member(new.gym_id) then
      raise exception 'You must be an active member of the gym to share this team';
    end if;
  end if;

  return new;
end;
$$;

-- ---------------------------------------------------------------------------
-- Atomic gym delete: clear shares, then delete (cascades members/invites/subs)
-- ---------------------------------------------------------------------------

create or replace function public.delete_gym(p_gym_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if auth.uid() is null then
    raise exception 'You must be signed in.';
  end if;

  if not exists (
    select 1
    from public.gym_members gm
    where gm.gym_id = p_gym_id
      and gm.coach_id = auth.uid()
      and gm.role = 'owner'
      and gm.status = 'active'
  ) then
    raise exception 'Only the gym owner can delete this gym';
  end if;

  update public.clients
  set gym_id = null
  where gym_id = p_gym_id;

  update public.teams
  set gym_id = null
  where gym_id = p_gym_id;

  delete from public.gyms
  where id = p_gym_id;
end;
$$;

revoke all on function public.delete_gym(uuid) from public;
grant execute on function public.delete_gym(uuid) to authenticated;

comment on function public.delete_gym(uuid) is
  'Owner-only gym dissolve: unassigns clients/teams from the gym (primary coach unchanged), then deletes the gym.';
