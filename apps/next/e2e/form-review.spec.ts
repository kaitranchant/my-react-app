...
test.describe('Form review', () => {
  test('client can open form review and upload a photo', async ({
    clientPage: page,
  }) => {
    ...
    await page.locator('#form-review-file').setInputFiles(fixtureImage)
    await expect(
      page.getByRole('button', { name: 'progress-photo.png' })
    ).toBeVisible()
