...
export function formatSupabaseError(error: PostgrestError): string {
  ...
  if (
    error.code === 'PGRST205' &&
    error.message.includes('client_inbody_scans')
  ) {
    return 'InBody scans are not set up on this database yet. Run supabase/apply-client-inbody-scans.sql in the Supabase SQL editor, then try again.'
  }
