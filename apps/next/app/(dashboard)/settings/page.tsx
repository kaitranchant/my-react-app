...
export default async function SettingsPage() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select(
      'full_name, business_name, avatar_url, weight_unit, week_starts_on, coach_timezone, default_check_in_frequency, notify_check_ins, notify_form_reviews, notify_workout_completions, notify_missed_sessions, notify_invite_accepted, notify_prs, notify_weekly_summary'
    )
    .eq('id', user!.id)
    .single()
